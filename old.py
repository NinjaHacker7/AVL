...
def CheckAVL(tree):
    if  CheckHeight(tree) > 0:
        ...
        if abs(b) >= 2 or b != tree.getBalanceFactor():
            print("balance factor is {b} and tree claims it is {tree.getBalanceFactor()}")
            printTree(tree)
            return False
        else:
...
def Test(lib, seed=0, size=10, rounds=10, verbose=False):
    ...
    try:
        if  bst.getRight().getLeft().getRight().getData() != n+3:
            if verbose:
                print("Error: BST insert incorrect")
            flag = False
    except:
        if verbose:
            print("Error: BST insertion losing nodes")
        flag = False

    if verbose:
        if flag:
            print("BST insert test complete.")
        else:
            print("BST insert test failed.")
...
if __name__ == "__main__":
    ...
    print("Testing module {name} by {module.getName()}")    
    score=0
    for i in Test(module,seed=123456, size=1000, rounds=200, verbose=True):
        if i:
            score+=1

    print("Test result: {score}/5")
